...
const (
	...
	SourceInactiveReclaim            = store.LedgerSourceInactiveReclaim
	SourceRedistribution             = store.LedgerSourceRedistribution
	SourceRedistributionCompensation = store.LedgerSourceRedistributionCompensation
)
...
// Reclaim atomically subtracts a positive amount and writes a negative ledger row.
func (s *Service) Reclaim(ctx context.Context, req Request) (*Result, error) {
	if req.UserID <= 0 {
		return nil, fmt.Errorf("user id required")
	}
	if req.Amount <= 0 {
	...
	if req.Source == "" {
		req.Source = SourceInactiveReclaim
	}
	scope := req.Scope
	if scope == "" {
		scope = req.Source
	...
	user, err := s.client.SubtractBalanceScoped(ctx, scope, req.UserID, req.Amount, req.Notes, slot)
	if err != nil {
		_, _ = s.store.InsertLedger(ctx, store.LedgerEntry{
			UserID: req.UserID, Source: req.Source, SourceRef: req.SourceRef,
			Amount: -req.Amount, IdempotencyKey: key, Status: store.LedgerStatusFailed,
			Notes: req.Notes, Error: err.Error(),
		})
		return nil, err
	}
	bal := 0.0
	...
	if err != nil {
		return &Result{User: user, NewBalance: bal, IdemKey: key}, fmt.Errorf("reclaimed but ledger write failed: %w", err)
	}
	return &Result{LedgerID: id, User: user, NewBalance: bal, IdemKey: key}, nil
}
