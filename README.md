...
- 管理台创建活动 → 预览应付名单 → 确认结算；手工预览/结算可填写周期键（如 `2026-08-01`、`2026-W31`、`2026-08`）用于补发或重试
- 发奖明细支持周期、状态、用户 ID 筛选和分页，记录成功/失败/跳过、Ledger 流水及新发生失败的原因
- 结算前校验：日期合法、奖励规则有效、排行非空且存在应付名次；空榜/无应付会拒绝 settle
- 支持取消未完成结算的活动（已成功发放不回滚）；失败周期记录为 `partial` 后可按周期重试
- 永久删除仅允许没有任何发奖记录的活动；一旦产生发奖记录只能取消，服务端会再次校验
- 用户排行页展示进行中的活动横幅
- API：
  - 管理：`/ext/api/admin/rank/campaigns`（分页/筛选/创建）、`.../{id}`（查看/修改/安全删除）、`.../{id}/preview`、`.../{id}/settle`、`.../{id}/cancel`、`.../{id}/awards`（分页/筛选）
  - 公开：`GET /ext/api/ranking/campaigns`

### 额度回流（redistribution）
- 活跃判断只使用登录时间 `last_active_at` 和额度消耗时间 `last_used_at`；不再提供扩展行为、注册时长、当前余额、累计消耗等活跃条件
- 不活跃与活跃条件均支持“全部满足 / 任一满足”；新用户保护、最低余额、额度保底、充值用户保护仍属于资金安全规则
- 用户鉴权访问 `/ext/api/tasks` 时返回当前可分配额度池，任务中心顶部展示四位小数金额；匿名访问不会返回额度池
...
| `/ext/assets/rewards-icon.svg` | **我的奖励**侧栏菜单（推荐） |
